...
from fact_pscp_s11 where reportdate = '2021-12-19' and reporthour = 10  and procedure_type!=8 group by mme_address
) as a
...
from fact_pscp_s11 where reportdate = '2021-12-19' and reporthour = 10 and procedure_type!=8 group by sgw_address
) as a
...
select mme_address, count(1) times, sum(case when procedure_status = 3 then 1 else 0 end) fail,
sum(case when msisdn != '' then 1 else 0 end) msisdn_fill,sum(case when imsi != '' then 1 else 0 end) imsi_fill 
from fact_pscp_s6a where reportdate = '2021-12-19' and reporthour = 18  group by mme_address
) as a
'''
...
from fact_psup_http where reportdate='2021-12-19' and reporthour=10 and interface=11 group by source_ip
'''

spark_sql_list = [s1mme, s11_mme_dimension, s11_sgw_dimension, s6a, http]
